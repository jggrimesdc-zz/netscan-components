...
def handle(event, context):
    ...
    valid_path_parameters, body = RequestUtils.validate_digit_path_parameter(event["pathParameters"], "duns")
    if not valid_path_parameters:
        return RequestUtils.return_base64encoded_api_call(400, json.dumps(body))

    duns = event["pathParameters"]["duns"]
    ...
    status_code, body = WCaaSUtils.get_status_by_duns_for_scan_user(duns, user_id)
