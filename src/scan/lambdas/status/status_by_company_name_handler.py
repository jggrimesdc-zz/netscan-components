...
def handle(event, context):
    ...
    valid_path_parameters, body = RequestUtils.validate_path_parameter(event["pathParameters"], "company_name")

    if not valid_path_parameters:
        return RequestUtils.return_base64encoded_api_call(400, json.dumps(body))

    company_name = event["pathParameters"]["company_name"]
    ...
    status_code, body = WCaaSUtils.get_status_by_company_name_for_scan_user(company_name, user_id)
